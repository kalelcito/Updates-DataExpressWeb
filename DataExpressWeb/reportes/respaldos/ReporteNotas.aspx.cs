...
{
    public partial class RepNotas : System.Web.UI.Page
    {
                ...
                if (!String.IsNullOrEmpty(fechaInicial) && !String.IsNullOrEmpty(fechaFinal))
                {
                    if ((Convert.ToInt32(fechaInicial.Replace(@"/","")) < Convert.ToInt32(fechaFinal.Replace(@"/",""))) || (fechaInicial == fechaFinal))
                    {
                        ...
                        DB.CrearComando(@"set dateformat ymd SELECT  count(Dat_General.idComprobante)
                                     ...
                                     WHERE
                                        CONVERT(varchar,Dat_General.fecha,112)>=@fechaIni AND CONVERT(varchar,Dat_General.fecha,112)<=@fechaFin and Dat_General.codDoc = '04'");
                        DB.AsignarParametroCadena("@fechaIni", fechaInicial);
                        DB.AsignarParametroCadena("@fechaFin", fechaFinal);
                        ...
                        if (count > 0)
                        {
                            RepSucursal reporteSuc = new RepSucursal(dir, fecha, fechanom, "04", reporteindividual, reportesupervisor, empleado, sucursal);
                        ...
                        {
                            Label2.Text = "No se han elaborado notas de crédito durante estas fechas";
                        }
                    }
                    else
                    {
