...
// ***********************************************************************
// <copyright file="crearNomina.aspx.cs" company="DataExpress Latinoamérica">
...
namespace DataExpressWeb
{
    ...
    public partial class CrearNomina : Page
    {
        /// <summary>
        /// The _DB
        /// </summary>
        ...
        private Log _log;
        /// <summary>
        /// The suma percepciones
        /// </summary>
        private static decimal sumaPercepciones;
        /// <summary>
        /// The suma deducciones
        /// </summary>
        private static decimal sumaDeducciones;
        /// <summary>
        /// The suma isr
        /// </summary>
        private static decimal sumaISR;
        /// <summary>
        /// The valores isr
        /// </summary>
        private static List<string> valoresIsr;
        /// <summary>
        /// The list incap
        /// </summary>
        private static List<IncapacidadTemp> listIncap;
        /// <summary>
        /// The list extras
        /// </summary>
        private static List<HorasExtraTemp> listExtras;
        /// <summary>
        /// The Sucursal Emisor
        /// </summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session["IDENTEMI"] != null)
            {
                ...
                SqlDataMetodoPago.ConnectionString = _db.CadenaConexion;
                SqlDataPercTemp.ConnectionString = _db.CadenaConexion;
                SqlDataDedcTemp.ConnectionString = _db.CadenaConexion;
                ...
                SqlDataPercTemp.SelectParameters["SessionId"].DefaultValue = Session.SessionID;
                SqlDataDedcTemp.SelectParameters["SessionId"].DefaultValue = Session.SessionID;
                ...
                if (Session["idUser"] != null)
                {
                    ...
                    if (!Page.IsPostBack)
                    {
                        ...
                        listIncap = new List<IncapacidadTemp>();
                        listExtras = new List<HorasExtraTemp>();
                        //ddlMetodoPago_SelectedIndexChanged(null, null);
                    }
                    gvIncap.DataSource = listIncap;
                    gvExtras.DataSource = listExtras;
                }
            }
            else
            {
        ...
        {
            var selected = ddlMetodoPago.Items.Cast<ListItem>().Where(li => li.Selected).Select(li => li.Value).ToList();
            if (selected.Count <= 0)
            {
            ...
            try
            {
                ...
                txt.SetReceptorCfdi(tbRazonSocialRec.Text, tbRfcRec.Text, tbCURPR.Text, tbTelRec.Text, tbMailRec.Text, tbTelRec2.Text, tbDenomSocialRec.Text);
                if (cbDomRec.Checked)
                {
                ...
                if (valoresIsr.Count > 0)
                {
                    txt.SetCantidadImpuestosCfdi(CerosNull(sumaISR.ToString()));
                    foreach (var isr in valoresIsr)
                    {
                        txt.AgregaImpuestoRetencionCfdi("ISR", isr);
                    }
                }
                txt.AgregaConceptoCfdi("1", "SERVICIO", "", "PAGO DE NÓMINA", CerosNull(sumaPercepciones.ToString()), CerosNull(sumaPercepciones.ToString()));
                txt.SetComplemntoNominaCfdi(tbRegPatronalEmpleado.Text, tbIdEmpleado.Text, tbCurpEmpleado.Text, ddlRegimenEmpleado.SelectedValue, tbSegSocialEmpleado.Text, tbFechaPagoEmpleado.Text, tbFechaInicialPagoEmpleado.Text, tbFechaFinalPagoEmpleado.Text, tbDiasPagadosEmpleado.Text, tbDeptoEmpleado.Text, tbClabe.Text, ddlBancos.SelectedValue, tbFechaInicioRel.Text, tbAntiguedadEmpleado.Text, tbPuestoEmpleado
                    .Text, tbContratoEmpleado.Text, tbJornadaEmpleado.Text, tbPeriodicidadEmpleado.Text, tbSalarioBaseEmpleado.Text, ddlRiesgoPuesto.SelectedValue, tbSalarioDiarioEmpleado.Text);
                var sql1 = @"SELECT ISNULL(SUM(ImporteGravado), 0.00) AS gravado, ISNULL(SUM(ImporteExento), 0.00) AS exento FROM Dat_DetallesNominaTemp WHERE idUser = @ID AND codigo = @COD AND SessionId = @SessionId";
                var sql2 = @"SELECT * FROM Dat_DetallesNominaTemp WHERE idUser = @ID AND codigo = @COD AND SessionId = @SessionId";
                _db.Conectar();
                _db.CrearComando(sql1);
                _db.AsignarParametroCadena("@ID", _idUser);
                _db.AsignarParametroCadena("@SessionId", Session.SessionID);
                _db.AsignarParametroCadena("@COD", "1");
                var dr = _db.EjecutarConsulta();
                if (dr.Read())
                {
                    txt.SetNominaPercepciones(dr["gravado"].ToString(), dr["exento"].ToString());
                }
                _db.Desconectar();
                _db.Conectar();
                _db.CrearComando(sql1);
                _db.AsignarParametroCadena("@ID", _idUser);
                _db.AsignarParametroCadena("@SessionId", Session.SessionID);
                _db.AsignarParametroCadena("@COD", "2");
                dr = _db.EjecutarConsulta();
                if (dr.Read())
                {
                    txt.SetNominaDeducciones(dr["gravado"].ToString(), dr["exento"].ToString());
                }
                _db.Desconectar();
                _db.Conectar();
                _db.CrearComando(sql2);
                _db.AsignarParametroCadena("@ID", _idUser);
                _db.AsignarParametroCadena("@SessionId", Session.SessionID);
                _db.AsignarParametroCadena("@COD", "1");
                dr = _db.EjecutarConsulta();
                while (dr.Read())
                {
                    txt.AgregarPercepcionNomina(dr["Tipo"].ToString(), dr["Clave"].ToString(), dr["Concepto"].ToString(), dr["ImporteGravado"].ToString(), dr["ImporteExento"].ToString());
                }
                _db.Desconectar();
                _db.Conectar();
                _db.CrearComando(sql2);
                ...
                while (dr.Read())
                {
                    txt.AgregarDeduccionNomina(dr["Tipo"].ToString(), dr["Clave"].ToString(), dr["Concepto"].ToString(), dr["ImporteGravado"].ToString(), dr["ImporteExento"].ToString());
                }
                _db.Desconectar();
                foreach (var incap in listIncap)
                {
                    txt.AgregarIncapacidadNomina(incap.dias, incap.tipo, incap.descuento);
                }
                foreach (var hora in listExtras)
                {
                    txt.AgregarHorasExtraNomina(hora.dias, hora.tipo, hora.horas, hora.importe);
                }
                var tipocomp = "egreso";
                var ambiente = false;
                ...
                txt.SetComprobanteCfdi(ddlSerie.SelectedItem.Text, "", Localization.Now.ToString("s"), tbFormaPago.Text, tbCondPago.Text, tbSubtotal.Text, tbDescuento.Text, tbMotivoDescuento.Text, tbTipoCambio.Text, ddlMoneda.SelectedValue, tbTotal.Text, tipocomp, string.Join(",", selected), tbLugarExp.Text, tbNumCtaPago.Text);
                ...
                var result = coreMx.RecibeInfoTxt(txtInvoice, _idUser, Session["IDENTEMI"].ToString(), ambiente, "08", false, true, "", "");
                if (result != null)
                {
        ...
        private void Llenarlistadom(string rfc, string tipo, bool chkDom = true)
        {
            ...
            if (tipo == "emi")
            {
                ...
                //tbLocEmi.ReadOnly = control;
                //tbRefEmi.ReadOnly = control;
            ...
            if (dr.Read())
            {
                if (tipo == "emi")
                {
                    ...
                    //tbLocEmi.Text = dr["localidad"].ToString();
                    //tbRefEmi.Text = dr["referencia"].ToString();
            ...
            if (dr.HasRows)
            {
                while (dr.Read())
                {
                    ...
                    if (tipo == "rec")
                    {
                        ...
                        tbRazonSocialRec.Text = dr["NOMREC"].ToString();
                        tbCURPR.Text = dr["curp"].ToString();
                        tbCurpEmpleado.Text = dr["curp"].ToString();
                        tbMailRec.Text = dr["email"].ToString();
                        tbTelRec.Text = dr["telefono"].ToString();
                        var metodosPago = dr["metodoPago"].ToString().Split(',');
                        ddlMetodoPago.ClearSelection();
        ...
        /// <summary>
        /// Handles the Click event of the bAgregarPercepcion control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs" /> instance containing the event data.</param>
        protected void bAgregarPercepcion_Click(object sender, EventArgs e)
        {
            decimal gravado = 0;
            decimal exento = 0;
            decimal.TryParse(CerosNull(tbIGravadoPercepcion.Text), out gravado);
            decimal.TryParse(CerosNull(tbIExentoPercepcion.Text), out exento);
            var sql = @"INSERT INTO Dat_DetallesNominaTemp
                                   (idUser
                                   ,codigo
                                   ,Tipo
                                   ,Clave
            ...
            try
            {
                ...
                _db.CrearComando(sql);
                _db.AsignarParametroCadena("@idUser", _idUser);
                _db.AsignarParametroCadena("@codigo", "1");
                _db.AsignarParametroCadena("@Tipo", ddlTipoPercepcion.SelectedValue);
                _db.AsignarParametroCadena("@Clave", tbClavePercepcion.Text);
                _db.AsignarParametroCadena("@Concepto", tbConceptoPercepcion.Text);
                _db.AsignarParametroCadena("@ImporteGravado", CerosNull(tbIGravadoPercepcion.Text));
                _db.AsignarParametroCadena("@ImporteExento", CerosNull(tbIExentoPercepcion.Text));
                _db.AsignarParametroCadena("@SessionId", Session.SessionID);
                _db.EjecutarConsulta1();
                _db.Desconectar();
                ddlTipoPercepcion.SelectedIndex = 0;
                tbClavePercepcion.Text = "";
                tbConceptoPercepcion.Text = "";
                tbIExentoPercepcion.Text = "0.00";
                tbIGravadoPercepcion.Text = "0.00";
                SqlDataPercTemp.DataBind();
                gvPercepciones.DataBind();
                calcularTotales();
            }
            catch (Exception ex)
            {
                (Master as SiteMaster).MostrarAlerta(this, "No se pudo agregar la percepción:<br/>" + ex.Message, 4, null);
            }
        }
        ...
        protected void calcularTotales()
        {
            ...
            decimal _gravado2 = 0;
            decimal _exento2 = 0;
            var sql = @"SELECT Tipo, ISNULL(ImporteGravado, 0.00) AS gravado, ISNULL(ImporteExento, 0.00) AS exento FROM Dat_DetallesNominaTemp WHERE idUser = @ID AND codigo = @COD AND SessionId = @SessionId";
            try
            {
                _db.Conectar();
                _db.CrearComando(sql);
                ...
                while (dr.Read())
                {
                    decimal gravado = 0;
                    decimal exento = 0;
                    decimal.TryParse(dr["gravado"].ToString(), out gravado);
                    decimal.TryParse(dr["exento"].ToString(), out exento);
                    _gravado1 += gravado;
                    _exento1 += exento;
                    sumaPercepciones += (gravado + exento);
                }
                _db.Desconectar();
                _db.Conectar();
                _db.CrearComando(sql);
                ...
                while (dr.Read())
                {
                    ...
                    _gravado2 += gravado;
                    _exento2 += exento;
                    if (dr["Tipo"].ToString().Equals("002")) // 002 = ISR
                    {
                        sumaISR += gravado;
                        valoresIsr.Add(CerosNull(gravado.ToString()));
                    }
                    else
                    {
                        sumaDeducciones += (gravado + exento);
                    }
                }
            }
            catch (Exception)
            {
                //(Master as SiteMaster).MostrarAlerta(this, "No se pudo agregar la percepción:<br/>" + ex.Message, "Error");
            }
            finally
            {
                _db.Desconectar();
                tbTotalGravadoPercepciones.Text = CerosNull(_gravado1.ToString());
                tbTotalExentoPercepciones.Text = CerosNull(_exento1.ToString());
                tbTotalGravadoDeducciones.Text = CerosNull(_gravado2.ToString());
                tbTotalExentoDeducciones.Text = CerosNull(_exento2.ToString());
                tbSubtotal.Text = CerosNull(sumaPercepciones.ToString());
                tbDescuento.Text = CerosNull((sumaDeducciones).ToString());
                tbIva16.Text = CerosNull(sumaISR.ToString());
                tbTotal.Text = CerosNull((sumaPercepciones - sumaDeducciones - sumaISR).ToString());
                tbDescuentoP.Text = CerosNull(((sumaDeducciones * 100) / sumaPercepciones).ToString());
            }
        }

        /// <summary>
        /// Handles the Click event of the bAgregarDeduccion control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs" /> instance containing the event data.</param>
        protected void bAgregarDeduccion_Click(object sender, EventArgs e)
        {
            var sql = @"INSERT INTO Dat_DetallesNominaTemp
                                   (idUser
                                   ,codigo
            ...
            try
            {
                ...
                _db.CrearComando(sql);
                _db.AsignarParametroCadena("@idUser", _idUser);
                _db.AsignarParametroCadena("@codigo", "2");
                _db.AsignarParametroCadena("@Tipo", ddlTipoDeduccion.SelectedValue);
                _db.AsignarParametroCadena("@Clave", tbClaveDeduccion.Text);
                _db.AsignarParametroCadena("@Concepto", tbConceptoDeduccion.Text);
                _db.AsignarParametroCadena("@ImporteGravado", CerosNull(tbIGravadoDeduccion.Text));
                _db.AsignarParametroCadena("@ImporteExento", CerosNull(tbIExentoDeduccion.Text));
                _db.AsignarParametroCadena("@SessionId", Session.SessionID);
                _db.EjecutarConsulta1();
                _db.Desconectar();
                ddlTipoDeduccion.SelectedIndex = 0;
                ddlTipoDeduccion_SelectedIndexChanged(null, null);
                tbClaveDeduccion.Text = "";
                tbConceptoDeduccion.Text = "";
                tbIExentoDeduccion.Text = "0.00";
                tbIGravadoDeduccion.Text = "0.00";
                SqlDataDedcTemp.DataBind();
        ...
        /// <summary>
        /// Handles the RowDeleted event of the gvPercepciones control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="GridViewDeletedEventArgs" /> instance containing the event data.</param>
        protected void gvPercepciones_RowDeleted(object sender, GridViewDeletedEventArgs e)
        {
            calcularTotales();
        }

        /// <summary>
        /// Handles the RowDeleted event of the gvDeducciones control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="GridViewDeletedEventArgs" /> instance containing the event data.</param>
        protected void gvDeducciones_RowDeleted(object sender, GridViewDeletedEventArgs e)
        {
            calcularTotales();
        }

        /// <summary>
        /// Handles the SelectedIndexChanged event of the ddlTipoDeduccion control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs" /> instance containing the event data.</param>
        protected void ddlTipoDeduccion_SelectedIndexChanged(object sender, EventArgs e)
        {
            tbIExentoDeduccion.ReadOnly = ddlTipoDeduccion.SelectedValue.Equals("002");
            if (ddlTipoDeduccion.SelectedValue.Equals("002"))
            {
                tbIExentoDeduccion.Text = "0.00";
            }
        }
        ...
        /// <summary>
        /// Class IncapacidadTemp.
        /// </summary>
        public class IncapacidadTemp
        {
            /// <summary>
            ...
            /// <summary>
            /// Gets or sets the dias.
            /// </summary>
            /// <value>The dias.</value>
            ...
            /// <summary>
            /// Gets or sets the tipo string.
            /// </summary>
            /// <value>The tipo string.</value>
        ...
        /// <summary>
        /// Class HorasExtraTemp.
        /// </summary>
        public class HorasExtraTemp
        {
            /// <summary>
            /// Gets or sets the index.
            /// </summary>
        ...
        /// <summary>
        /// Handles the Click event of the bAgregarIncap control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs" /> instance containing the event data.</param>
        protected void bAgregarIncap_Click(object sender, EventArgs e)
        {
            listIncap.Add(new IncapacidadTemp() { index = listIncap.Count, dias = tbDiasIncap.Text, tipo = ddlTipoIncap.SelectedValue, tipoString = ddlTipoIncap.SelectedItem.Text, descuento = CerosNull(tbDescuentoIncap.Text) });
            gvIncap.DataBind();
            tbDiasIncap.Text = "";
            ddlTipoIncap.SelectedValue = "0";
            tbDescuentoIncap.Text = "";
        }

        /// <summary>
        /// Handles the RowDeleting event of the gvIncap control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="GridViewDeleteEventArgs" /> instance containing the event data.</param>
        protected void gvIncap_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            int rowIndex = Convert.ToInt32(e.Keys["index"]);
            listIncap.RemoveAt(rowIndex);
            gvIncap.DataBind();
        }

        /// <summary>
        /// Handles the PageIndexChanging event of the gvIncap control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="GridViewPageEventArgs" /> instance containing the event data.</param>
        protected void gvIncap_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            gvIncap.PageIndex = e.NewPageIndex;
            gvIncap.DataBind();
        }

        /// <summary>
        /// Handles the Click event of the bAgregarExtras control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs" /> instance containing the event data.</param>
        protected void bAgregarExtras_Click(object sender, EventArgs e)
        {
            listExtras.Add(new HorasExtraTemp() { index = listExtras.Count, dias = tbDiasIncap.Text, tipo = ddlTipoExtras.SelectedItem.Text, horas = tbHorasExtras.Text, importe = CerosNull(tbDescuentoIncap.Text) });
            gvExtras.DataBind();
            tbDiasExtras.Text = "";
            ddlTipoExtras.SelectedValue = "0";
            tbHorasExtras.Text = "";
            tbImporteExtras.Text = "";
        }

        /// <summary>
        /// Handles the RowDeleting event of the gvExtras control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="GridViewDeleteEventArgs" /> instance containing the event data.</param>
        protected void gvExtras_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            int rowIndex = Convert.ToInt32(e.Keys["index"]);
            listExtras.RemoveAt(rowIndex);
            gvExtras.DataBind();
        }

        /// <summary>
        /// Handles the PageIndexChanging event of the gvExtras control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="GridViewPageEventArgs" /> instance containing the event data.</param>
        protected void gvExtras_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            gvExtras.PageIndex = e.NewPageIndex;
            gvExtras.DataBind();
        }
