...
// ***********************************************************************
// <copyright file="crearNotaCredito.aspx.cs" company="DataExpress Latinoamérica">
...
{
    /// <summary>
    /// Class CrearNotaCredito.
    /// </summary>
    /// <seealso cref="System.Web.UI.Page" />
    public partial class CrearNotaCredito : Page
    {
        /// <summary>
        /// The _ish p
        /// </summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["IDENTEMI"] != null)
            {
                ...
                if (Session["idUser"] != null)
                {
                    ...
                    chkHabilitarIvaRet10.Attributes.Add("checkStrings", "10.00%|0%");
                    divIvaRet10.Style["display"] = Session["IDGIRO"].ToString().Contains("1") && Regex.IsMatch(Session["IDENTEMI"].ToString(), @"TCA130827M58|LAN7008173R5") ? "block" : "none";
                    _db.Conectar();
        ...
        protected void FinishButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (Session["IDENTEMI"].ToString().Equals("LAN7008173R5") && Session["CfdiVersion"].Equals("3.3"))
                //{
                //    var result = coreMx.RecibeInfoTxtPreviewPagos(txtInvoice, _idUser, Session["IDENTEMI"].ToString(), ambiente, "01", false, true);
                //    if (result != null)
                //    {
                //        var cfdi = result[0];
                //        var pagos = result[1];
                ...
                //            xDoc.LoadXml(cfdi.OuterXml);
                //            Session["uuidCreado"] = GetAtributte(xDoc, "UUID", "tfd:TimbreFiscalDigital");
                //            (Master as SiteMaster).MostrarAlerta(this, "El comprobante se ha generado satisfactoriamente", 2, null, "window.location.href = '" + ResolveUrl("~/Documentos.aspx") + "';");
                //            if (pagos != null)
                //            {
                //                var xDocPago = new XmlDocument();
                //                xDocPago.LoadXml(pagos.OuterXml);
                //                var bytesEncoded = Encoding.UTF8.GetBytes(xDocPago.OuterXml);
                //                var base64 = Convert.ToBase64String(bytesEncoded);
                //                var contentType = "application/octet-stream";
                //                var uuidPago = GetAtributte(xDocPago, "UUID", "tfd:TimbreFiscalDigital");
                //                var fileName = "XmlPago_" + uuidPago + ".xml";
                //                ScriptManager.RegisterStartupScript(this, GetType(), "_btnXmlPago", "downloadBytes('" + base64 + "','" + contentType + "','" + fileName + "', false);", true);
                //            }
                //        }
        ...
        protected void CalcularTotales(object sender = null, EventArgs e = null)
        {
            ...
            decimal ish = 0;
            var sql = @"SELECT ISNULL(SUM(precioTotalSinImpuestos), 0.00) AS subtotal, ISNULL(SUM(descuento), 0.00) as iva16, ISNULL(SUM(ish), 0.00) AS ish, ISNULL(SUM(CONVERT(FLOAT, descuentoCfdi33)), 0.00) as descuento FROM Dat_DetallesTemp WHERE (id_Empleado = @idUser AND SessionId = @SessionId)";
            _db.Conectar();
            ...
            if (dr.Read())
            {
                ...
                decimal.TryParse(CerosNull(dr["ish"].ToString()), out ish);
            ...
            {
                decimal tasa = 0;
                decimal.TryParse(CerosNull(dr["tarifa"].ToString()), out tasa);
            ...
            {
                decimal val = 0;
                decimal.TryParse(CerosNull(dr["total"].ToString()), out val);
                impuestosRet += val;
            }
            _db.Desconectar();
            ...
            total = subTotal - descuento + impuestosTra - impuestosRet + impuestosTraLoc - impuestosRetLoc + iva16 + ish;
        ...
        protected void bAgregarConcepto_Click(object sender, EventArgs e)
        {
            ...
            var sql = @"INSERT INTO Dat_DetallesTemp
                                    ...
                                    ,id_Empleado,SessionId,PrecioTotalSinImpuestos,unidad,ish" + (!string.IsNullOrEmpty(tbNumeroPredial.Text) ? ",ctaPredial" : "") + (chkHabilitarIva16.Checked ? ",descuento" : "") + @",claveProdServ,claveUnidad,descuentoCfdi33) OUTPUT inserted.idDetallesTemp
                                    VALUES
                                    (" + (!string.IsNullOrEmpty(tbIdentConc.Text) ? "@codigoPrincipal," : "") + @"@descripcion,@cantidad,@precioUnitario
                                   ,@id_Empleado,@SessionId,@PrecioTotalSinImpuestos,@unidad,@ish" + (!string.IsNullOrEmpty(tbNumeroPredial.Text) ? ",@ctaPredial" : "") + (chkHabilitarIva16.Checked ? ",@iva16" : "") + @",@claveProdServ,@claveUnidad,@descuentoCfdi33)";
            var claveProdServ = "";
            var claveUnidad = "";
            ...
            if (!string.IsNullOrEmpty(idConcepto))
            {
                ...
                _db.EjecutarConsulta1();
                _db.Desconectar();
                if (chkHabilitarIvaRet10.Checked)
                {
                    decimal tasaIvaRet = (decimal)0.10;
                    ...
                    _db.AsignarParametroCadena("@tarifa", CerosNull(tasaIvaRet.ToString()));
            ...
            chkHabilitarIvaRet10.Checked = false;
        ...
        protected void tbRfcRec_TextChanged(object sender, EventArgs e)
        {
            ...
            LlenaContactosInicio();
            if (Session["IDENTEMI"].ToString().Equals("SET920324FC3", StringComparison.OrdinalIgnoreCase))
            {
                divAddendaCts.Style["display"] = tbRfcRec.Text.Equals("CTS021101KU8", StringComparison.OrdinalIgnoreCase) || tbRfcRec.Text.Equals("CTS960918BJ4", StringComparison.OrdinalIgnoreCase) ? "inline" : "none";
            }
        ...
        private string BuildTxt()
        {
            ...
            _db.Desconectar();
            sql = "SELECT idDetallesTemp, codigoPrincipal AS noId, descripcion, precioUnitario AS valorUnitario, cantidad, precioTotalSinImpuestos AS importe, unidad, ctaPredial, descuento as iva16, ish, claveProdServ, claveUnidad, descuentoCfdi33 FROM Dat_DetallesTemp WHERE (id_Empleado = @idUser AND SessionId = @SessionId)";
            _db.Conectar();
            _db.CrearComando(sql);
            _db.AsignarParametroCadena("@idUser", _idUser);
            _db.AsignarParametroCadena("@SessionId", Session.SessionID);
            ...
            foreach (var concepto in conceptos)
            {
                if (Session["_CfdiRelacionados"] != null)
                {
                    ...
                    if (lista.Count > 0)
                    {
                        txt.AgregaConceptoCfdi(concepto[4].ToString(), concepto[6].ToString(), concepto[1].ToString(), concepto[2].ToString(), concepto[3].ToString(), concepto[5].ToString(), concepto[12].ToString(), "84111506", "ACT", concepto[0].ToString());
                    }
                    else
                    {
            ...
            txt.SetImpuestosLocalesCfdi(tbTotalRetLoc.Text, CerosNull(ish.ToString()));
            #region NODO INE
            if (chkHabilitarINE.Checked)
            {
        ...
        private bool ExisteTicket()
        {
            ...
            {
                _db.Conectar();
                _db.CrearComando("SELECT idComprobante FROM Dat_General WHERE (folioReservacion = @folio OR noTicket = @folio) AND codDoc = '04' AND estado = 1 AND tipo = 'E'");
        ...
        private bool ExisteReservacion()
        {
            ...
            if (Session["IDENTEMI"].ToString().Equals("IMO700623PJ4") || Session["IDENTEMI"].ToString().Equals("HAP9504215L5") || Session["IDENTEMI"].ToString().Equals("HSC0010193M7") || Session["IDENTEMI"].ToString().Equals("OAP981214DP3") || Session["IDENTEMI"].ToString().Equals("OHF921110BF2") || Session["IDENTEMI"].ToString().Equals("OHS0312152Z4") || Session["IDENTEMI"].ToString().Equals("EOP000815CM5") || Session["IDENTEMI"].ToString().Equals("OHE070511872") || Session["IDENTEMI"].ToString().Equals("IPS0806248X5"))
            {
                ...
                {
                    _db.Conectar();
                    _db.CrearComando("SELECT idComprobante FROM Dat_General WHERE folioReservacion = @folio AND codDoc = '04' AND estado = 1 AND tipo = 'E'");
                    _db.AsignarParametroCadena("@folio", folio);
                    var dr = _db.EjecutarConsulta();
                    existe = dr.Read();
                    _db.Desconectar();
        ...
        protected void lbAddendas_Click(object sender, EventArgs e)
        {
            ...
            if (Session["IDGIRO"].ToString().Contains("1") && Regex.IsMatch(Session["IDENTEMI"].ToString(), @"LAN7008173R5|SET920324FC3|YAM8302185A8|OHF921110BF2") && Regex.IsMatch(tbRfcRec.Text, @"VCI0004041R0"))
            {
                divAddendasTlalnepantla.Visible = Session["IDGIRO"].ToString().Contains("1") && Regex.IsMatch(Session["IDENTEMI"].ToString(), @"LAN7008173R5|SET920324FC3|YAM8302185A8|OHF921110BF2") && Regex.IsMatch(tbRfcRec.Text, @"VCI0004041R0");
                divSinAddendas.Visible = !divAddendasTlalnepantla.Visible;
            }
            //divAddendasTlalnepantla.Visible = Session["IDGIRO"].ToString().Contains("1") && Regex.IsMatch(Session["IDENTEMI"].ToString(), @"LAN7008173R5|SET920324FC3") && Regex.IsMatch(tbRfcRec.Text, @"VCI0004041R0");
            // divAddendasTlalnepantla.Visible = Session["IDGIRO"].ToString().Contains("1") && Regex.IsMatch(Session["IDENTEMI"].ToString(), @"LAN7008173R5|SET920324FC3") && Regex.IsMatch(tbRfcRec.Text, @"VCI0004041R0");
            // divSinAddendas.Visible = !divAddendasTlalnepantla.Visible;
